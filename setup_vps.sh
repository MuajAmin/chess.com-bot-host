...
SETUP_MARKER="$BOT_DIR/.setup_done"

# ── Fast path: skip heavy setup if already done ──────────────
if [ -f "$SETUP_MARKER" ]; then
    echo "[✓] Initial setup already complete — skipping system packages."
    echo "    (Delete $SETUP_MARKER to force a full re-setup)"
else
    # --- System Update ---
...
# Only install requirements if they changed
REQ_HASH=$(md5sum "$BOT_DIR/requirements.txt" 2>/dev/null | cut -d' ' -f1)
OLD_HASH=""
if [ -f "$BOT_DIR/.req_hash" ]; then
    OLD_HASH=$(cat "$BOT_DIR/.req_hash")
fi
...
if [ "$REQ_HASH" != "$OLD_HASH" ]; then
    ...
    echo "$REQ_HASH" > "$BOT_DIR/.req_hash"
else
    echo "    [✓] Python dependencies unchanged — skipping pip install."
fi

# Only install Playwright browser if not already present
if [ ! -d "/home/bot/.cache/ms-playwright" ]; then
...
echo "[7/7] Checking Lc0..."
LC0_VERSION="0.31.2"

if command -v lc0 &>/dev/null; then
    echo "    [✓] Lc0 already installed: $(lc0 --help 2>&1 | head -1 || echo 'found')"
else
    echo "    Downloading Lc0 v${LC0_VERSION}..."
